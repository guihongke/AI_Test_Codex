 ...
 */
public interface VRefreshComponent extends VOnStateChangedListener {
    /**
     * 获取实体视图
     * @return 实体视图
     */
    ...
    /**
     * 获取变换方式 {@link VSpinnerStyle} 必须返回 非空
     * @return 变换方式
     */
    @NonNull
    VSpinnerStyle getSpinnerStyle();
    ...
    /**
     * 【仅限框架内调用】尺寸定义完成 （如果高度不改变（代码修改：setHeader），只调用一次, 在VRefreshLayout#onMeasure中调用）
     * @param kernel VRefreshKernel
     * @param height HeaderHeight or FooterHeight
     * @param maxDragHeight 最大拖动高度
     */
    @RestrictTo({LIBRARY,LIBRARY_GROUP,SUBCLASSES})
    void onInitialized(@NonNull VRefreshKernel kernel, int height, int maxDragHeight);
    /**
     ...
     */
    @RestrictTo({LIBRARY,LIBRARY_GROUP,SUBCLASSES})
    void onReleased(@NonNull VRefreshLayout refreshLayout, int height, int maxDragHeight);
     ...
     */
    @RestrictTo({LIBRARY,LIBRARY_GROUP,SUBCLASSES})
    void onStartAnimator(@NonNull VRefreshLayout refreshLayout, int height, int maxDragHeight);
     ...
     */
    @RestrictTo({LIBRARY,LIBRARY_GROUP,SUBCLASSES})
    int onFinish(@NonNull VRefreshLayout refreshLayout, boolean success);
