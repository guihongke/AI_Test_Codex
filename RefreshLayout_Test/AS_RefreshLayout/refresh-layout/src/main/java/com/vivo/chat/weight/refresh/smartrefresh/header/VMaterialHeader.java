...
import com.vivo.chat.weight.refresh.smartrefresh.header.material.VCircleImageView;
import com.vivo.chat.weight.refresh.smartrefresh.header.material.VMaterialProgressDrawable;
...
public class VMaterialHeader extends VSimpleComponent implements VRefreshHeader {
    ...
    protected VRefreshKernel mKernel;
    protected VMaterialProgressDrawable mProgress;
    ...
    protected VRefreshState mState;
    protected boolean mShowBezierWave = false;
    protected boolean mScrollableWhenRefreshing = true;

    //<editor-fold desc="VMaterialHeader">
    public VMaterialHeader(Context context) {
        this(context, null);
    }

    public VMaterialHeader(Context context, AttributeSet attrs) {
        super(context, attrs, 0);

        mSpinnerStyle = VSpinnerStyle.MatchLayout;
        final View thisView = this;
        final ViewGroup thisGroup = this;
        thisView.setMinimumHeight(VSmartUtil.dp2px(100));

        mProgress = new VMaterialProgressDrawable(this);
//        mProgress.setBackgroundColor(CIRCLE_BG_LIGHT);
//        mProgress.setAlpha(255);
        mProgress.setColorSchemeColors(0xff0099cc,0xffff4444,0xff669900,0xffaa66cc,0xffff8800);
        mCircleView = new VCircleImageView(context, CIRCLE_BG_LIGHT);
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.VMaterialHeader);

        mShowBezierWave = ta.getBoolean(R.styleable.VMaterialHeader_srlShowBezierWave, mShowBezierWave);
        mScrollableWhenRefreshing = ta.getBoolean(R.styleable.VMaterialHeader_srlScrollableWhenRefreshing, mScrollableWhenRefreshing);
        mBezierPaint.setColor(ta.getColor(R.styleable.VMaterialHeader_srlPrimaryColor, 0xff11bbff));
        if (ta.hasValue(R.styleable.VMaterialHeader_srlShadowRadius)) {
            int radius = ta.getDimensionPixelOffset(R.styleable.VMaterialHeader_srlShadowRadius, 0);
            int color = ta.getColor(R.styleable.VMaterialHeader_mhShadowColor, 0xff000000);
            mBezierPaint.setShadowLayer(radius, 0, 0, color);
            thisView.setLayerType(LAYER_TYPE_SOFTWARE, null);
        }

        mShowBezierWave = ta.getBoolean(R.styleable.VMaterialHeader_mhShowBezierWave, mShowBezierWave);
        mScrollableWhenRefreshing = ta.getBoolean(R.styleable.VMaterialHeader_mhScrollableWhenRefreshing, mScrollableWhenRefreshing);
        if (ta.hasValue(R.styleable.VMaterialHeader_mhPrimaryColor)) {
            mBezierPaint.setColor(ta.getColor(R.styleable.VMaterialHeader_mhPrimaryColor, 0xff11bbff));
        }
        if (ta.hasValue(R.styleable.VMaterialHeader_mhShadowRadius)) {
            int radius = ta.getDimensionPixelOffset(R.styleable.VMaterialHeader_mhShadowRadius, 0);
            int color = ta.getColor(R.styleable.VMaterialHeader_mhShadowColor, 0xff000000);
            mBezierPaint.setShadowLayer(radius, 0, 0, color);
            thisView.setLayerType(LAYER_TYPE_SOFTWARE, null);
        }
     ...
     */
    public VMaterialHeader setProgressBackgroundColorSchemeResource(@ColorRes int colorRes) {
        final View thisView = this;
     ...
     * @param color 颜色
     */
    public VMaterialHeader setProgressBackgroundColorSchemeColor(@ColorInt int color) {
...
//        mProgress.setBackgroundColor(color);
     ...
     * @return VMaterialHeader
     */
    public VMaterialHeader setColorSchemeColors(@ColorInt int... colors) {
     ...
     * @return VMaterialHeader
     */
    public VMaterialHeader setColorSchemeResources(@ColorRes int... colorIds) {
        final View thisView = this;
        final Context context = thisView.getContext();
     ...
     * @return VMaterialHeader
     */
    public VMaterialHeader setSize(int size) {
     ...
     * @return VMaterialHeader
     */
    public VMaterialHeader setShowBezierWave(boolean show) {
     ...
     */
    public VMaterialHeader setScrollableWhenRefreshing(boolean scrollable) {
