...
import com.vivo.chat.weight.refresh.smartrefresh.util.VDesignUtil;
...
import static com.vivo.chat.weight.refresh.smartrefresh.util.VSmartUtil.isContentView;
import static com.vivo.chat.weight.refresh.smartrefresh.util.VSmartUtil.isTransformedTouchPointInView;
import static com.vivo.chat.weight.refresh.smartrefresh.util.VSmartUtil.measureViewHeight;
import static com.vivo.chat.weight.refresh.smartrefresh.util.VSmartUtil.scrollListBy;
...
public class VRefreshContentWrapper implements VRefreshContent, VCoordinatorLayoutListener, AnimatorUpdateListener {
    ...
    protected VSimpleBoundaryDecider mBoundaryAdapter = new VSimpleBoundaryDecider();

    public VRefreshContentWrapper(@NonNull View view) {
        this.mContentView = mOriginalContentView = mScrollableView = view;
    }

    //<editor-fold desc="findScrollableView">
    protected void findScrollableView(View content, VRefreshKernel kernel) {
        ...
        while (scrollableView == null || (scrollableView instanceof NestedScrollingParent
            ...
            if (!isInEditMode) {
                VDesignUtil.checkCoordinatorLayout(content, kernel, this);
            }
...
//            mBoundaryAdapter.setActionEvent(mMotionEvent);
    ...
    @Override
    public void setUpComponent(VRefreshKernel kernel, View fixedHeader, View fixedFooter) {
    ...
    @Override
    public void setScrollBoundaryDecider(VScrollBoundaryDecider boundary) {
        if (boundary instanceof VSimpleBoundaryDecider) {
            mBoundaryAdapter = ((VSimpleBoundaryDecider) boundary);
        } else {
