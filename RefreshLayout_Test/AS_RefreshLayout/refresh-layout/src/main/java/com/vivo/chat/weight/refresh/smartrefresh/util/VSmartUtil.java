 ...
 */
public class VSmartUtil implements Interpolator {
    ...
    public static boolean isScrollableView(View view) {
        if (view instanceof VRefreshComponent) {
            return false;
        }
    ...
    public static boolean canLoadMore(@NonNull View targetView, PointF touch, boolean contentFull) {
        ...
        if (targetView instanceof ViewGroup && touch != null && !VSmartUtil.isScrollableView(targetView)) {
            ViewGroup viewGroup = (ViewGroup) targetView;
            final int childCount = viewGroup.getChildCount();
            PointF point = new PointF();
            for (int i = childCount; i > 0; i--) {
