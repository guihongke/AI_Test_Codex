...
import com.vivo.chat.weight.refresh.smartrefresh.api.VRefreshContent;
...
import com.vivo.chat.weight.refresh.smartrefresh.constant.VDimensionStatus;
...
import com.vivo.chat.weight.refresh.smartrefresh.R;
import com.vivo.chat.weight.refresh.smartrefresh.listener.VDefaultRefreshFooterCreator;
import com.vivo.chat.weight.refresh.smartrefresh.listener.VDefaultRefreshHeaderCreator;
import com.vivo.chat.weight.refresh.smartrefresh.listener.VDefaultRefreshInitializer;
...
import com.vivo.chat.weight.refresh.smartrefresh.simple.VSimpleBoundaryDecider;
import com.vivo.chat.weight.refresh.smartrefresh.util.VSmartUtil;
import com.vivo.chat.weight.refresh.smartrefresh.wrapper.VRefreshContentWrapper;
import com.vivo.chat.weight.refresh.smartrefresh.wrapper.VRefreshFooterWrapper;
import com.vivo.chat.weight.refresh.smartrefresh.wrapper.VRefreshHeaderWrapper;
...
import static com.vivo.chat.weight.refresh.smartrefresh.util.VSmartUtil.dp2px;
import static com.vivo.chat.weight.refresh.smartrefresh.util.VSmartUtil.fling;
...
/**
 * 智能刷新布局
 * Intelligent VRefreshLayout
 * Created by scwang on 2017/5/26.
 */
@SuppressWarnings({"unused"})
@SuppressLint("RestrictedApi")
public class VSmartRefreshLayout extends ViewGroup implements VRefreshLayout, NestedScrollingParent/*, NestedScrollingChild*/ {
    ...
    protected VOnRefreshListener mRefreshListener;
    protected VOnLoadMoreListener mLoadMoreListener;
    protected VOnMultiListener mOnMultiListener;
    protected VScrollBoundaryDecider mScrollBoundaryDecider;
    //</editor-fold>
    ...
    protected VDimensionStatus mHeaderHeightStatus = VDimensionStatus.DefaultUnNotify;
    protected int mFooterHeight;        //底部高度 和 底部高度状态
    protected VDimensionStatus mFooterHeightStatus = VDimensionStatus.DefaultUnNotify;
    ...
    protected VRefreshComponent mRefreshHeader;     //下拉头部视图
    protected VRefreshComponent mRefreshFooter;     //上拉底部视图
    /**
     * 显示内容视图
     * 根据开发设计，本变量最终不可能为空，因为在 onAttachedToWindow 中判断如果未设置内容视图
     * 会自动创建一个 TextView 提示必须创建，并且 TextView 会变为 内容视图
     */
    protected VRefreshContent mRefreshContent;
    //</editor-fold>

    protected Paint mPaint;
    protected Handler mHandler;
    protected VRefreshKernel mKernel = new VRefreshKernelImpl();

    /**
     * 【主要状态】
     * 面对 SmartRefresh 外部的滚动状态
     */
    protected VRefreshState mState = VRefreshState.None;          //主状态
    /**
     * 【附加状态】
     ...
     */
    protected VRefreshState mViceState = VRefreshState.None;      //副状态（主状态刷新时候的滚动状态）
    ...
    /*
     * https://github.com/scwang90/VSmartRefreshLayout/issues/1540
    ...
    //全局默认 Footer 构造器
    protected static VDefaultRefreshFooterCreator sFooterCreator = null;
    //全局默认 Header 构造器
    protected static VDefaultRefreshHeaderCreator sHeaderCreator = null;
    //全局默认 控件 初始化器
    protected static VDefaultRefreshInitializer sRefreshInitializer = null;
    //默认全局 布局 Margin
    ...
    public VSmartRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        mReboundInterpolator = new VSmartUtil(VSmartUtil.INTERPOLATOR_VISCOUS_FLUID);
        mTouchSlop = configuration.getScaledTouchSlop();
        mMinimumVelocity = configuration.getScaledMinimumFlingVelocity();
        mMaximumVelocity = configuration.getScaledMaximumFlingVelocity();

        mFooterHeight = VSmartUtil.dp2px(60);
        mHeaderHeight = VSmartUtil.dp2px(100);

        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.VSmartRefreshLayout);

        /*
         * VSmartRefreshLayout 继承 ViewGroup 后即使不设置 android:clipToPadding，也等效于 android:clipToPadding=true
         * 特意添加 android:clipToPadding 来在 Java 代码中 判断是否设置过，没有设置强制 setClipToPadding(false)
         * android:clipChildren 也是同样
         */
        if (!ta.hasValue(R.styleable.VSmartRefreshLayout_android_clipToPadding)) {
            super.setClipToPadding(false);
        }
        if (!ta.hasValue(R.styleable.VSmartRefreshLayout_android_clipChildren)) {
        ...
        mFooterTriggerRate = ta.getFloat(R.styleable.VSmartRefreshLayout_srlFooterTriggerRate, mFooterTriggerRate);
        mEnableRefresh = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnableRefresh, mEnableRefresh);
        mReboundDuration = ta.getInt(R.styleable.VSmartRefreshLayout_srlReboundDuration, mReboundDuration);
        mEnableLoadMore = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnableLoadMore, mEnableLoadMore);
        mHeaderHeight = ta.getDimensionPixelOffset(R.styleable.VSmartRefreshLayout_srlHeaderHeight, mHeaderHeight);
        mFooterHeight = ta.getDimensionPixelOffset(R.styleable.VSmartRefreshLayout_srlFooterHeight, mFooterHeight);
        mHeaderInsetStart = ta.getDimensionPixelOffset(R.styleable.VSmartRefreshLayout_srlHeaderInsetStart, mHeaderInsetStart);
        mFooterInsetStart = ta.getDimensionPixelOffset(R.styleable.VSmartRefreshLayout_srlFooterInsetStart, mFooterInsetStart);
        mDisableContentWhenRefresh = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlDisableContentWhenRefresh, mDisableContentWhenRefresh);
        mDisableContentWhenLoading = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlDisableContentWhenLoading, mDisableContentWhenLoading);
        mEnableHeaderTranslationContent = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnableHeaderTranslationContent, mEnableHeaderTranslationContent);
        mEnableFooterTranslationContent = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnableFooterTranslationContent, mEnableFooterTranslationContent);
        mEnablePreviewInEditMode = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnablePreviewInEditMode, mEnablePreviewInEditMode);
        mEnableAutoLoadMore = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnableAutoLoadMore, mEnableAutoLoadMore);
        mEnableOverScrollBounce = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnableOverScrollBounce, mEnableOverScrollBounce);
        mEnablePureScrollMode = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnablePureScrollMode, mEnablePureScrollMode);
        mEnableScrollContentWhenLoaded = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnableScrollContentWhenLoaded, mEnableScrollContentWhenLoaded);
        mEnableScrollContentWhenRefreshed = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnableScrollContentWhenRefreshed, mEnableScrollContentWhenRefreshed);
        mEnableLoadMoreWhenContentNotFull = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnableLoadMoreWhenContentNotFull, mEnableLoadMoreWhenContentNotFull);
        mEnableFooterFollowWhenNoMoreData = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnableFooterFollowWhenLoadFinished, mEnableFooterFollowWhenNoMoreData);
        mEnableFooterFollowWhenNoMoreData = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnableFooterFollowWhenNoMoreData, mEnableFooterFollowWhenNoMoreData);
        mEnableClipHeaderWhenFixedBehind = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnableClipHeaderWhenFixedBehind, mEnableClipHeaderWhenFixedBehind);
        mEnableClipFooterWhenFixedBehind = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnableClipFooterWhenFixedBehind, mEnableClipFooterWhenFixedBehind);
        mEnableOverScrollDrag = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnableOverScrollDrag, mEnableOverScrollDrag);
        mFixedHeaderViewId = ta.getResourceId(R.styleable.VSmartRefreshLayout_srlFixedHeaderViewId, mFixedHeaderViewId);
        mFixedFooterViewId = ta.getResourceId(R.styleable.VSmartRefreshLayout_srlFixedFooterViewId, mFixedFooterViewId);
        mHeaderTranslationViewId = ta.getResourceId(R.styleable.VSmartRefreshLayout_srlHeaderTranslationViewId, mHeaderTranslationViewId);
        mFooterTranslationViewId = ta.getResourceId(R.styleable.VSmartRefreshLayout_srlFooterTranslationViewId, mFooterTranslationViewId);
        mEnableNestedScrolling = ta.getBoolean(R.styleable.VSmartRefreshLayout_srlEnableNestedScrolling, mEnableNestedScrolling);
        mNestedChild.setNestedScrollingEnabled(mEnableNestedScrolling);

        mManualLoadMore = mManualLoadMore || ta.hasValue(R.styleable.VSmartRefreshLayout_srlEnableLoadMore);
        mManualHeaderTranslationContent = mManualHeaderTranslationContent || ta.hasValue(R.styleable.VSmartRefreshLayout_srlEnableHeaderTranslationContent);
        mManualFooterTranslationContent = mManualFooterTranslationContent || ta.hasValue(R.styleable.VSmartRefreshLayout_srlEnableFooterTranslationContent);
        mHeaderHeightStatus = ta.hasValue(R.styleable.VSmartRefreshLayout_srlHeaderHeight) ? VDimensionStatus.XmlLayoutUnNotify : mHeaderHeightStatus;
        mFooterHeightStatus = ta.hasValue(R.styleable.VSmartRefreshLayout_srlFooterHeight) ? VDimensionStatus.XmlLayoutUnNotify : mFooterHeightStatus;

        int accentColor = ta.getColor(R.styleable.VSmartRefreshLayout_srlAccentColor, 0);
        int primaryColor = ta.getColor(R.styleable.VSmartRefreshLayout_srlPrimaryColor, 0);
        if (primaryColor != 0) {
            if (accentColor != 0) {
    ...
    protected void onFinishInflate() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (VSmartUtil.isContentView(view) && (contentLevel < 2 || i == 1)) {
                // 可滚动内容
                indexContent = i;
                contentLevel = 2; // 标记为可滚动内容，不会再被替换
            } else if (!(view instanceof VRefreshComponent) && contentLevel < 1) {
                // 普通内容
                indexContent = i;
        ...
        int indexFooter = -1;
        if (indexContent >= 0) {
            mRefreshContent = new VRefreshContentWrapper(super.getChildAt(indexContent));
            if (indexContent == 1) {
                indexHeader = 0;
                if (count == 3) {
                    indexFooter = 2;
        ...
        for (int i = 0; i < count; i++) {
            View view = super.getChildAt(i);
            if (i == indexHeader || (i != indexFooter && indexHeader == -1 && mRefreshHeader == null && view instanceof VRefreshHeader)) {
                mRefreshHeader = (view instanceof VRefreshHeader) ? (VRefreshHeader) view : new VRefreshHeaderWrapper(view);
            } else if (i == indexFooter || (indexFooter == -1 && view instanceof VRefreshFooter)) {
                mEnableLoadMore = (mEnableLoadMore || !mManualLoadMore);
                mRefreshFooter = (view instanceof VRefreshFooter) ? (VRefreshFooter) view : new VRefreshFooterWrapper(view);
            }
        }
        ...
        if (!thisView.isInEditMode()) {
            ...
            if (mRefreshHeader == null) {
                if (sHeaderCreator != null) {
                    VRefreshHeader header = sHeaderCreator.createRefreshHeader(thisView.getContext(), this);
                    //noinspection ConstantConditions
                    if (header == null) {
                        throw new RuntimeException("VDefaultRefreshHeaderCreator can not return null");
                    }
            ...
            if (mRefreshFooter == null) {
                if (sFooterCreator != null) {
                    VRefreshFooter footer = sFooterCreator.createRefreshFooter(thisView.getContext(), this);
                    //noinspection ConstantConditions
                    if (footer == null) {
                        throw new RuntimeException("VDefaultRefreshFooterCreator can not return null");
                    }
            ...
            if (mRefreshContent == null) {
                for (int i = 0, len = getChildCount(); i < len; i++) {
                    ...
                    if ((mRefreshHeader == null || view != mRefreshHeader.getView()) &&
                        ...
                        mRefreshContent = new VRefreshContentWrapper(view);
                    }
                }
            }
            if (mRefreshContent == null) {
                final int padding = VSmartUtil.dp2px(20);
                ...
                mRefreshContent = new VRefreshContentWrapper(errorView);
            ...
            if (mSpinner != 0) {
                notifyStateChanged(VRefreshState.None);
     ...
     * 1.测量代码看起来很复杂，时因为 Header Footer 有四种拉伸变换样式 {@link VSpinnerStyle}，每一种样式有自己的测量方法
    ...
    protected void onMeasure(final int widthMeasureSpec,final int heightMeasureSpec) {
        ...
        for (int i = 0, len = super.getChildCount(); i < len; i++) {
            ...
            if (mRefreshHeader != null && mRefreshHeader.getView() == child) {
                ...
                if (mHeaderHeightStatus.ordinal < VDimensionStatus.XmlLayoutUnNotify.ordinal) {
                    if (lp.height > 0) {
                        height =  lp.height + mlp.bottomMargin + mlp.topMargin;
                        if (mHeaderHeightStatus.canReplaceWith(VDimensionStatus.XmlExactUnNotify)) {
                            mHeaderHeight = lp.height + mlp.bottomMargin + mlp.topMargin;
                            mHeaderHeightStatus = VDimensionStatus.XmlExactUnNotify;
                        }
                    } else if (lp.height == WRAP_CONTENT && (mRefreshHeader.getSpinnerStyle() != VSpinnerStyle.MatchLayout || !mHeaderHeightStatus.notified)) {
                        ...
                        if (measuredHeight > 0) {
                            height = -1;
                            if (measuredHeight != (maxHeight) && mHeaderHeightStatus.canReplaceWith(VDimensionStatus.XmlWrapUnNotify)) {
                                mHeaderHeight = measuredHeight + mlp.bottomMargin + mlp.topMargin;
                                mHeaderHeightStatus = VDimensionStatus.XmlWrapUnNotify;
                            }
                        }
                    }
                }

                if (mRefreshHeader.getSpinnerStyle() == VSpinnerStyle.MatchLayout) {
            ...
            if (mRefreshFooter != null && mRefreshFooter.getView() == child) {
                ...
                if (mFooterHeightStatus.ordinal < VDimensionStatus.XmlLayoutUnNotify.ordinal) {
                    if (lp.height > 0) {
                        height = lp.height + mlp.topMargin + mlp.bottomMargin;
                        if (mFooterHeightStatus.canReplaceWith(VDimensionStatus.XmlExactUnNotify)) {
                            mFooterHeight = lp.height + mlp.topMargin + mlp.bottomMargin;
                            mFooterHeightStatus = VDimensionStatus.XmlExactUnNotify;
                        }
                    } else if (lp.height == WRAP_CONTENT && (mRefreshFooter.getSpinnerStyle() != VSpinnerStyle.MatchLayout || !mFooterHeightStatus.notified)) {
                        ...
                        if (measuredHeight > 0) {
                            height = -1;
                            if (measuredHeight != (maxHeight) && mFooterHeightStatus.canReplaceWith(VDimensionStatus.XmlWrapUnNotify)) {
                                mFooterHeight = measuredHeight + mlp.topMargin + mlp.bottomMargin;
                                mFooterHeightStatus = VDimensionStatus.XmlWrapUnNotify;
                            }
                        }
                    }
                }

                if (mRefreshFooter.getSpinnerStyle() == VSpinnerStyle.MatchLayout) {
                    height = getSize(heightMeasureSpec);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0, len = super.getChildCount(); i < len; i++) {
            ...
            if (mRefreshHeader != null && mRefreshHeader.getView() == child) {
                ...
                if (!isPreviewMode) {
                    if (mRefreshHeader.getSpinnerStyle() == VSpinnerStyle.Translate) {
                        top = top - mHeaderHeight;
                        bottom = bottom - mHeaderHeight;
                        /*
                         * VSpinnerStyle.Scale  headerView.getMeasuredHeight() 已经重复处理
                         **/
...
//                        bottom = top + Math.max(Math.max(0, isEnableRefreshOrLoadMore(mEnableRefresh) ? mSpinner : 0) - lp.bottomMargin - lp.topMargin, 0);
            ...
            if (mRefreshFooter != null && mRefreshFooter.getView() == child) {
                ...
                final ViewGroup.LayoutParams lp = footerView.getLayoutParams();
                final MarginLayoutParams mlp = lp instanceof MarginLayoutParams ? (MarginLayoutParams)lp : sDefaultMarginLP;
                final VSpinnerStyle style = mRefreshFooter.getSpinnerStyle();
                int left = mlp.leftMargin;
                ...
                if (mFooterNoMoreData && mFooterNoMoreDataEffective && mEnableFooterFollowWhenNoMoreData && mRefreshContent != null
                        && mRefreshFooter.getSpinnerStyle() == VSpinnerStyle.Translate
                ...
                if (style == VSpinnerStyle.MatchLayout) {
                    top = mlp.topMargin - mFooterInsetStart;
                } else if (isPreviewMode
                        || style == VSpinnerStyle.FixedFront
                        || style == VSpinnerStyle.FixedBehind) {
    ...
    protected void onDetachedFromWindow() {
        ...
        /*
         * 2020-5-27
         * https://github.com/scwang90/VSmartRefreshLayout/issues/1166
         * 修复 Fragment 脱离屏幕再回到时，菊花转圈，无法关闭的问题。
         * Smart 脱离屏幕时，必须重置状态，清空mHandler，否则动画等效果会导致 APP 内存泄露
         */
        if (mRefreshHeader != null && mState == VRefreshState.Refreshing) {
            mRefreshHeader.onFinish(this, false);
        }
        if (mRefreshFooter != null && mState == VRefreshState.Loading) {
            mRefreshFooter.onFinish(this, false);
        }
        if (mSpinner != 0) {
            mKernel.moveSpinner(0, true);
        }
        if (mState != VRefreshState.None) {
            notifyStateChanged(VRefreshState.None);
        }
        if (mHandler != null) {
        ...
        /*
         * https://github.com/scwang90/VSmartRefreshLayout/issues/716
         * 在一些特殊情况下，当触发上拉加载更多后，
        ...
        if (mRefreshHeader != null && mRefreshHeader.getView() == child) {
            ...
            if (contentView != null) {
                ...
                if (mHeaderBackgroundColor != 0 && mPaint != null) {
                    ...
                    } else if (mRefreshHeader.getSpinnerStyle() == VSpinnerStyle.Translate) {
                 ...
                 */
                if ((mEnableClipHeaderWhenFixedBehind && mRefreshHeader.getSpinnerStyle() == VSpinnerStyle.FixedBehind) || mRefreshHeader.getSpinnerStyle().scale) {
                    canvas.save();
            ...
            if (contentView != null) {
                ...
                if (mFooterBackgroundColor != 0 && mPaint != null) {
                    ...
                    if (mRefreshFooter.getSpinnerStyle().scale) {
                        top = child.getTop();
                    } else if (mRefreshFooter.getSpinnerStyle() == VSpinnerStyle.Translate) {
                 ...
                 * 并且取消了对 child 的绘制裁剪，所以经典组件需要重写 dispatchDraw 自行裁剪
                 */
                if ((mEnableClipFooterWhenFixedBehind && mRefreshFooter.getSpinnerStyle() == VSpinnerStyle.FixedBehind) || mRefreshFooter.getSpinnerStyle().scale) {
                    canvas.save();
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        if (interceptAnimatorByAction(action)
                || (mState == VRefreshState.Loading && mDisableContentWhenLoading)
                || (mState == VRefreshState.Refreshing && mDisableContentWhenRefresh)) {
            return false;
        }
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (mState == VRefreshState.TwoLevel && mTouchY < thisView.getMeasuredHeight() * (1 - mTwoLevelBottomPullUpToCloseRate)) {
                    mDragDirection = 'h';//二级刷新标记水平滚动来禁止拖动
                    return mSuperDispatchTouchEvent;
                }
                if (mRefreshContent != null) {
                    //为 VRefreshContent 传递当前触摸事件的坐标，用于智能判断对应坐标位置View的滚动边界和相关信息
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!mIsBeingDragged && !mEnableDisallowIntercept && mDragDirection != 'h' && mRefreshContent != null) {//没有拖动之前，检测  canRefresh canLoadMore 来开启拖动
                    if (mDragDirection == 'v' || (Math.abs(dy) >= mTouchSlop && Math.abs(dx) < Math.abs(dy))) {//滑动允许最大角度为45度
                        ...
                        } else if (dy < 0 && (mSpinner > 0 || ((mEnableOverScrollDrag || mEnableLoadMore) && ((mState==VRefreshState.Loading&&mFooterLocked)||mRefreshContent.canLoadMore())))) {
                            mIsBeingDragged = true;
                            mTouchY = touchY + mTouchSlop;//调整 mTouchSlop 偏差
                        }
                        if (mIsBeingDragged) {
                            ...
                            mKernel.setState((mSpinner > 0 || (mSpinner == 0 && dy > 0)) ? VRefreshState.PullDownToRefresh : VRefreshState.PullUpToLoad);
                            final ViewParent parent = thisView.getParent();
                            if (parent instanceof ViewGroup) {
                                //修复问题 https://github.com/scwang90/VSmartRefreshLayout/issues/580
...
//        }
        if (Math.abs(velocity) > mMinimumVelocity) {
            if (velocity * mSpinner < 0) {
                 ...
                 */
                if (mState == VRefreshState.Refreshing || mState == VRefreshState.Loading || (mSpinner < 0 && mFooterNoMoreData)) {
            ...
            if ((velocity < 0 && ((mEnableOverScrollBounce && (mEnableLoadMore || mEnableOverScrollDrag)) || (mState == VRefreshState.Loading && mSpinner >= 0) || (mEnableAutoLoadMore&&isEnableRefreshOrLoadMore(mEnableLoadMore))))
                    || (velocity > 0 && ((mEnableOverScrollBounce && mEnableRefresh || mEnableOverScrollDrag) || (mState == VRefreshState.Refreshing && mSpinner <= 0)))) {
                /*
    ...
    protected boolean interceptAnimatorByAction(int action) {
        if (action == MotionEvent.ACTION_DOWN) {
            if (reboundAnimator != null) {
                if (mState.isFinishing || mState == VRefreshState.TwoLevelReleased || mState == VRefreshState.RefreshReleased || mState == VRefreshState.LoadReleased) {
                    return true;//完成动画和打开动画不能被打断
                }
                if (mState == VRefreshState.PullDownCanceled) {
                    mKernel.setState(VRefreshState.PullDownToRefresh);
                } else if (mState == VRefreshState.PullUpCanceled) {
                    mKernel.setState(VRefreshState.PullUpToLoad);
                }
                reboundAnimator.setDuration(0);//cancel会触发End调用，可以判断0来确定是否被cancel
                reboundAnimator.cancel();//会触发 cancel 和 end 调用
    ...
    protected void notifyStateChanged(VRefreshState state) {
        final VRefreshState oldState = mState;
        if (oldState != state) {
            mState = state;
            mViceState = state;
            final VOnStateChangedListener refreshHeader = mRefreshHeader;
            final VOnStateChangedListener refreshFooter = mRefreshFooter;
            final VOnStateChangedListener refreshListener = mOnMultiListener;
            ...
            if (state == VRefreshState.LoadFinish) {
                mFooterLocked = false;
            }
    ...
    protected void setStateDirectLoading(boolean triggerLoadMoreEvent) {
        if (mState != VRefreshState.Loading) {
            mLastOpenTime = currentTimeMillis();
//            if (mState != VRefreshState.LoadReleased) {
//                if (mState != VRefreshState.ReleaseToLoad) {
//                    if (mState != VRefreshState.PullUpToLoad) {
//                        mKernel.setState(VRefreshState.PullUpToLoad);
//                    }
//                    mKernel.setState(VRefreshState.ReleaseToLoad);
//                }
//                notifyStateChanged(VRefreshState.LoadReleased);
//                if (mRefreshFooter != null) {
...
//            }
            ...
            notifyStateChanged(VRefreshState.Loading);
            ...
            if (mOnMultiListener != null && mRefreshFooter instanceof VRefreshFooter) {
                final VOnLoadMoreListener listener = mOnMultiListener;
                if (triggerLoadMoreEvent) {
                    listener.onLoadMore(this);
                }
                final float maxDragHeight = mFooterMaxDragRate < 10 ? mFooterHeight * mFooterMaxDragRate : mFooterMaxDragRate;
                mOnMultiListener.onFooterStartAnimator((VRefreshFooter) mRefreshFooter, mFooterHeight, (int) maxDragHeight);
            }
        }
    }
    ...
    protected void setStateLoading(final boolean notify) {
        ...
        if (mOnMultiListener != null && mRefreshFooter instanceof VRefreshFooter) {
            ...
            final float maxDragHeight = mFooterMaxDragRate < 10 ? mFooterHeight * mFooterMaxDragRate : mFooterMaxDragRate;
            mOnMultiListener.onFooterReleased((VRefreshFooter) mRefreshFooter, mFooterHeight, (int) maxDragHeight);
        }
        if (animator == null) {
    ...
    protected void setStateRefreshing(final boolean notify) {
        AnimatorListenerAdapter listener = new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                mLastOpenTime = currentTimeMillis();
                notifyStateChanged(VRefreshState.Refreshing);
                if (mRefreshListener != null) {
                    if(notify) {
                        mRefreshListener.onRefresh(VSmartRefreshLayout.this);
                    }
                } else if (mOnMultiListener == null) {
                    finishRefresh(3000);
                }
                if (mRefreshHeader != null) {
                    ...
                    mRefreshHeader.onStartAnimator(VSmartRefreshLayout.this, mHeaderHeight,  maxDragHeight);
                }
                if (mOnMultiListener != null && mRefreshHeader instanceof VRefreshHeader) {
                    if (notify) {
                        mOnMultiListener.onRefresh(VSmartRefreshLayout.this);
                    }
                    final int maxDragHeight = (int)(mHeaderMaxDragRate < 10 ? mHeaderHeight * mHeaderMaxDragRate : mHeaderMaxDragRate);
                    mOnMultiListener.onHeaderStartAnimator((VRefreshHeader) mRefreshHeader, mHeaderHeight,  maxDragHeight);
                }
            }
        };
        notifyStateChanged(VRefreshState.RefreshReleased);
        ...
        if (mOnMultiListener != null && mRefreshHeader instanceof VRefreshHeader) {
            ...
            final int maxDragHeight = (int)(mHeaderMaxDragRate < 10 ? mHeaderHeight * mHeaderMaxDragRate : mHeaderMaxDragRate);
            mOnMultiListener.onHeaderReleased((VRefreshHeader)mRefreshHeader, mHeaderHeight,  maxDragHeight);
        }
        if (animator == null) {
...
//    }
     ...
     * @param state 状态
     */
    protected void setViceState(VRefreshState state) {
     ...
     * @return enable
     */
    protected boolean isEnableTranslationContent(boolean enable, @Nullable VRefreshComponent internal) {
        /*
         * 2019-12-25 修复 2.0 版本之后无默认 Header Footer 导致的纯滚动模式无效 添加 @Nullable
         */
        return enable || mEnablePureScrollMode || internal == null || internal.getSpinnerStyle() == VSpinnerStyle.FixedBehind;
    }
    ...
    protected class FlingRunnable implements Runnable {
        ...
        public Runnable start() {
            ...
            if (mSpinner != 0 && (!(mState.isOpening || (mFooterNoMoreData && mEnableFooterFollowWhenNoMoreData && mFooterNoMoreDataEffective && isEnableRefreshOrLoadMore(mEnableLoadMore)))
                    || ((mState == VRefreshState.Loading || (mFooterNoMoreData && mEnableFooterFollowWhenNoMoreData && mFooterNoMoreDataEffective && isEnableRefreshOrLoadMore(mEnableLoadMore))) && mSpinner < -mFooterHeight)
                    || (mState == VRefreshState.Refreshing && mSpinner > mHeaderHeight))) {
                int frame = 0;
                int offset = mSpinner;
                ...
                while (spinner * offset > 0) {
                    ...
                    if (Math.abs(velocityFrame) < 1) {
                        if (!mState.isOpening
                                || (mState == VRefreshState.Refreshing && offset > mHeaderHeight)
                                || (mState != VRefreshState.Refreshing && offset < -mFooterHeight)) {
                            return null;
                        }
                        break;
                    }
        ...
        public void run() {
            if (animationRunnable == this && !mState.isFinishing) {
                ...
                if (Math.abs(velocity) >= 1) {
                    ...
                    animationRunnable = null;
                    if (Math.abs(mSpinner) >= Math.abs(mSmoothDistance)) {
                        int duration = 10 * Math.min(Math.max((int) VSmartUtil.px2dp(Math.abs(mSpinner-mSmoothDistance)), 30), 100);
        ...
        if (mSpinner != endSpinner) {
            ...
            reboundAnimator.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    reboundAnimator = null;
                    if (mSpinner == 0 && mState != VRefreshState.None && !mState.isOpening && !mState.isDragging) {
                        notifyStateChanged(VRefreshState.None);
...
//            reboundAnimator.setDuration(20000);
    ...
    protected void animSpinnerBounce(final float velocity) {
        if (reboundAnimator == null) {
            if (velocity > 0 && (mState == VRefreshState.Refreshing || mState == VRefreshState.TwoLevel)) {
                animationRunnable = new BounceRunnable(velocity, mHeaderHeight);
            } else if (velocity < 0 && (mState == VRefreshState.Loading
                    || (mEnableFooterFollowWhenNoMoreData && mFooterNoMoreData && mFooterNoMoreDataEffective && isEnableRefreshOrLoadMore(mEnableLoadMore))
                    || (mEnableAutoLoadMore && !mFooterNoMoreData && isEnableRefreshOrLoadMore(mEnableLoadMore) && mState != VRefreshState.Refreshing))) {
    ...
    protected void overSpinner() {
        if (mState == VRefreshState.TwoLevel) {
            final View thisView = this;
        ...
        } else if (mState == VRefreshState.Loading
        ...
        } else if (mState == VRefreshState.Refreshing) {
            ...
            mKernel.setState(VRefreshState.PullUpCanceled);
        } else if (mState == VRefreshState.ReleaseToRefresh) {
            mKernel.setState(VRefreshState.Refreshing);
        } else if (mState == VRefreshState.ReleaseToLoad) {
            mKernel.setState(VRefreshState.Loading);
        } else if (mState == VRefreshState.ReleaseToTwoLevel) {
            mKernel.setState(VRefreshState.TwoLevelReleased);
        } else if (mState == VRefreshState.RefreshReleased) {
            if (reboundAnimator == null) {
                mKernel.animSpinner(mHeaderHeight);
            }
        } else if (mState == VRefreshState.LoadReleased) {
            if (reboundAnimator == null) {
                mKernel.animSpinner(-mFooterHeight);
            }
        } else if (mState == VRefreshState.LoadFinish) {
            /*
             * 2020-5-26 修复 finishLoadMore 中途
             * 拖拽导致 状态重置 最终导致 显示 NoMoreData Footer 菊花却任然在转的情况
             * overSpinner 时 LoadFinish 状态无任何操作即可
             */
            Log.d("VSmartRefreshLayout", "overSpinner 时 LoadFinish 状态无任何操作即可");
        } else if (mSpinner != 0) {
            mKernel.animSpinner(0);
        }
    }
    ...
    protected void moveSpinnerInfinitely(float spinner) {
         ...
         *
         * 1.全局关闭（推荐）
         *         VSmartRefreshLayout.setDefaultRefreshInitializer(new VDefaultRefreshInitializer() {
         *             @Override
         *             public void initialize(@NonNull Context context, @NonNull VRefreshLayout layout) {
         *                 layout.getLayout().setTag("close egg");
         *             }
         *         });
         *
         * 2.XML关闭
         *          <com.vivo.chat.weight.refresh.smartrefresh.VSmartRefreshLayout
        ...
        if (mState == VRefreshState.TwoLevel && spinner > 0) {
            mKernel.moveSpinner(Math.min((int) spinner, thisView.getMeasuredHeight()), true);
        } else if (mState == VRefreshState.Refreshing && spinner >= 0) {
        ...
        } else if (spinner < 0 && (mState == VRefreshState.Loading
                || (mEnableFooterFollowWhenNoMoreData && mFooterNoMoreData && mFooterNoMoreDataEffective && isEnableRefreshOrLoadMore(mEnableLoadMore))
        ...
        if (mEnableAutoLoadMore && !mFooterNoMoreData && isEnableRefreshOrLoadMore(mEnableLoadMore) && spinner < 0
                && mState != VRefreshState.Refreshing
                && mState != VRefreshState.Loading
                && mState != VRefreshState.LoadFinish) {
            ...
            mHandler.postDelayed(() -> {
                if (mLoadMoreListener != null) {
                    mLoadMoreListener.onLoadMore(VSmartRefreshLayout.this);
                } else if (mOnMultiListener == null) {
                    finishLoadMore(2000);//如果没有任何加载监听器，两秒之后自动关闭
                }
                final VOnLoadMoreListener listener = mOnMultiListener;
                if (listener != null) {
                    listener.onLoadMore(VSmartRefreshLayout.this);
                }
            }, mReboundDuration);
        }
    }
...
//    }
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        public LayoutParams(Context context, AttributeSet attrs) {
            super(context, attrs);
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.VSmartRefreshLayout_Layout);
            backgroundColor = ta.getColor(R.styleable.VSmartRefreshLayout_Layout_layout_srlBackgroundColor, backgroundColor);
            if (ta.hasValue(R.styleable.VSmartRefreshLayout_Layout_layout_srlSpinnerStyle)) {
                spinnerStyle = VSpinnerStyle.values[ta.getInt(R.styleable.VSmartRefreshLayout_Layout_layout_srlSpinnerStyle, VSpinnerStyle.Translate.ordinal)];
            }
            ta.recycle();
        }
...
//        }
        ...
        public VSpinnerStyle spinnerStyle = null;
    }
    //</editor-fold>
    ...
    public void onNestedScroll(@NonNull View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed) {
        ...
        if ((dy < 0 && (mEnableRefresh || mEnableOverScrollDrag) && (mTotalUnconsumed != 0 || mScrollBoundaryDecider == null || mScrollBoundaryDecider.canRefresh(mRefreshContent.getView())))
            ...
            if (mViceState == VRefreshState.None || mViceState.isOpening) {
                /*
                 * 嵌套下拉或者上拉时，如果状态还是原始，需要更新到对应的状态
                 * mViceState.isOpening 时，主要修改的也是 mViceState 本身，而 mState 一直都是 isOpening
                 */
                mKernel.setState(dy > 0 ? VRefreshState.PullUpToLoad : VRefreshState.PullDownToRefresh);
                if (!scrolled) {
                    final View thisView = this;
                    final ViewParent parent = thisView.getParent();
                    if (parent != null) {
...
//        mManualNestedScrolling = true;
    ...
    public boolean isNestedScrollingEnabled() {
        /*
         * && 后面的判断是为了解决 https://github.com/scwang90/VSmartRefreshLayout/issues/961 问题
         */
...
//    }
     ...
     */
    @Override
    public VRefreshLayout setHeaderHeight(float heightDp) {
     ...
     */
    @Override
    public VRefreshLayout setHeaderHeightPx(int height) {
        if (height == mHeaderHeight) {
            return this;
        }
        if (mHeaderHeightStatus.canReplaceWith(VDimensionStatus.CodeExact)) {
            mHeaderHeight = height;
            if (mRefreshHeader != null && mAttachedToWindow && mHeaderHeightStatus.notified) {
                VSpinnerStyle style = mRefreshHeader.getSpinnerStyle();
                if (style != VSpinnerStyle.MatchLayout && !style.scale) {
                    /*
                     * 兼容 MotionLayout 2019-6-18
                     * 在 MotionLayout 内部 requestLayout 无效
                     * 该用 直接调用 layout 方式
                     * https://github.com/scwang90/VSmartRefreshLayout/issues/944
                     */
//                  mRefreshHeader.getView().requestLayout();
                    ...
                    int top = mlp.topMargin + mHeaderInsetStart - ((style == VSpinnerStyle.Translate) ? mHeaderHeight : 0);
                    headerView.layout(left, top, left + headerView.getMeasuredWidth(), top + headerView.getMeasuredHeight());
                }
                final int maxDragHeight = (int)(mHeaderMaxDragRate < 10 ? mHeaderHeight * mHeaderMaxDragRate : mHeaderMaxDragRate);
                mHeaderHeightStatus = VDimensionStatus.CodeExact;
                mRefreshHeader.onInitialized(mKernel, mHeaderHeight, maxDragHeight);
            } else {
                mHeaderHeightStatus = VDimensionStatus.CodeExactUnNotify;
            }
        }
        return this;
    }
     ...
     */
    @Override
    public VRefreshLayout setFooterHeight(float heightDp) {
     ...
     */
    @Override
    public VRefreshLayout setFooterHeightPx(int height) {
        if (height == mFooterHeight) {
            return this;
        }
        if (mFooterHeightStatus.canReplaceWith(VDimensionStatus.CodeExact)) {
            mFooterHeight = height;
            if (mRefreshFooter != null && mAttachedToWindow && mFooterHeightStatus.notified) {
                VSpinnerStyle style = mRefreshFooter.getSpinnerStyle();
                if (style != VSpinnerStyle.MatchLayout && !style.scale) {
                    /*
                     * 兼容 MotionLayout 2019-6-18
                     * 在 MotionLayout 内部 requestLayout 无效
...
//                  mRefreshFooter.getView().requestLayout();
                    ...
                    final int left = mlp.leftMargin;
                    final int top = mlp.topMargin + thisView.getMeasuredHeight() - mFooterInsetStart - ((style != VSpinnerStyle.Translate) ? mFooterHeight : 0);
                    footerView.layout(left, top, left + footerView.getMeasuredWidth(), top + footerView.getMeasuredHeight());
                }
                final float maxDragHeight = mFooterMaxDragRate < 10 ? mFooterHeight * mFooterMaxDragRate : mFooterMaxDragRate;
                mFooterHeightStatus = VDimensionStatus.CodeExact;
                mRefreshFooter.onInitialized(mKernel, mFooterHeight, (int) maxDragHeight);
            } else {
                mFooterHeightStatus = VDimensionStatus.CodeExactUnNotify;
            }
        }
        return this;
    }
     ...
     */
    @Override
    public VRefreshLayout setHeaderInsetStart(float insetDp) {
     ...
     */
    @Override
    public VRefreshLayout setHeaderInsetStartPx(int insetPx) {
     ...
     */
    @Override
    public VRefreshLayout setFooterInsetStart(float insetDp) {
     ...
     */
    @Override
    public VRefreshLayout setFooterInsetStartPx(int insetPx) {
     ...
     */
    @Override
    public VRefreshLayout setHeaderMaxDragRate(float rate) {
     ...
     */
    @Override
    public VRefreshLayout setFooterMaxDragRate(float rate) {
     ...
     */
    @Override
    public VRefreshLayout setHeaderTriggerRate(float rate) {
     ...
     */
    @Override
    public VRefreshLayout setFooterTriggerRate(float rate) {
     ...
     */
    @Override
    public VRefreshLayout setReboundInterpolator(@NonNull Interpolator interpolator) {
     ...
     */
    @Override
    public VRefreshLayout setReboundDuration(int duration) {
     ...
     */
    @Override
    public VRefreshLayout setEnableLoadMore(boolean enabled) {
     ...
     * 是否启用下拉刷新（默认启用）
     * @param enabled 是否启用
     * @return VSmartRefreshLayout
     */
    @Override
    public VRefreshLayout setEnableRefresh(boolean enabled) {
     ...
     */
    @Override
    public VRefreshLayout setEnableHeaderTranslationContent(boolean enabled) {
     ...
     */
    @Override
    public VRefreshLayout setEnableFooterTranslationContent(boolean enabled) {
     ...
     */
    @Override
    public VRefreshLayout setEnableAutoLoadMore(boolean enabled) {
     ...
     */
    @Override
    public VRefreshLayout setEnableOverScrollBounce(boolean enabled) {
     ...
     */
    @Override
    public VRefreshLayout setEnablePureScrollMode(boolean enabled) {
     ...
     */
    @Override
    public VRefreshLayout setEnableScrollContentWhenLoaded(boolean enabled) {
     ...
     */
    @Override
    public VRefreshLayout setEnableScrollContentWhenRefreshed(boolean enabled) {
     ...
     */
    @Override
    public VRefreshLayout setEnableLoadMoreWhenContentNotFull(boolean enabled) {
     ...
     */
    @Override
    public VRefreshLayout setEnableOverScrollDrag(boolean enabled) {
     ...
     */
    @Override
    public VRefreshLayout setEnableFooterFollowWhenNoMoreData(boolean enabled) {
     ...
     */
    @Override
    public VRefreshLayout setEnableClipHeaderWhenFixedBehind(boolean enabled) {
     ...
     */
    @Override
    public VRefreshLayout setEnableClipFooterWhenFixedBehind(boolean enabled) {
     ...
     */
    @Override
    public VRefreshLayout setEnableNestedScroll(boolean enabled) {
     ...
     */
    @Override
    public VRefreshLayout setFixedHeaderViewId(int id) {
     ...
     */
    @Override
    public VRefreshLayout setFixedFooterViewId(int id) {
     ...
     */
    @Override
    public VRefreshLayout setHeaderTranslationViewId(int id) {
     ...
     */
    @Override
    public VRefreshLayout setFooterTranslationViewId(int id) {
...
//     */
//    @Override
//    public VRefreshLayout setEnableNestedScrollOnly(boolean enabled) {
...
//    }
     ...
     */
    @Override
    public VRefreshLayout setDisableContentWhenRefresh(boolean disable) {
     ...
     */
    @Override
    public VRefreshLayout setDisableContentWhenLoading(boolean disable) {
     ...
     */
    @Override
    public VRefreshLayout setRefreshHeader(@NonNull VRefreshHeader header) {
     ...
     */
    @Override
    public VRefreshLayout setRefreshHeader(@NonNull VRefreshHeader header, int width, int height) {
        if (mRefreshHeader != null) {
        ...
        this.mHeaderHeightStatus = VDimensionStatus.DefaultUnNotify;//2020-5-23 修复动态切换时，不能及时测量新的高度
        /*
     ...
     */
    @Override
    public VRefreshLayout setRefreshFooter(@NonNull VRefreshFooter footer) {
     ...
     */
    @Override
    public VRefreshLayout setRefreshFooter(@NonNull VRefreshFooter footer, int width, int height) {
        if (mRefreshFooter != null) {
        ...
        this.mFooterHeightStatus = VDimensionStatus.DefaultUnNotify;//2020-5-23 修复动态切换时，不能及时测量新的高度
     ...
     */
    @Override
    public VRefreshLayout setRefreshContent(@NonNull View content) {
     ...
     */
    @Override
    public VRefreshLayout setRefreshContent(@NonNull View content, int width, int height) {
        final View thisView = this;
        if (mRefreshContent != null) {
        ...
        mRefreshContent = new VRefreshContentWrapper(content);
    ...
    @Nullable
    @Override
    public VRefreshFooter getRefreshFooter() {
        return mRefreshFooter instanceof VRefreshFooter ? (VRefreshFooter) mRefreshFooter : null;
    }
    ...
    @Nullable
    @Override
    public VRefreshHeader getRefreshHeader() {
        return mRefreshHeader instanceof VRefreshHeader ? (VRefreshHeader) mRefreshHeader : null;
    }
     ...
     */
    @Override
    public VRefreshLayout setOnRefreshListener(VOnRefreshListener listener) {
     ...
     */
    @Override
    public VRefreshLayout setOnLoadMoreListener(VOnLoadMoreListener listener) {
     ...
     */
    @Override
    public VRefreshLayout setOnRefreshLoadMoreListener(VOnRefreshLoadMoreListener listener) {
        this.mRefreshListener = listener;
        this.mLoadMoreListener = listener;
        this.mEnableLoadMore = mEnableLoadMore || (!mManualLoadMore && listener != null);
        return this;
     ...
     * Recommended {@link VSimpleBoundaryDecider}
     * 设置滚动边界判断器
     * 建议使用 {@link VSimpleBoundaryDecider}
     * @param listener VOnMultiListener 多功能监听器
     * @return VRefreshLayout
     */
    @Override
    public VRefreshLayout setOnMultiListener(VOnMultiListener listener) {
     ...
     */
    @Override
    public VRefreshLayout setPrimaryColors(@ColorInt int... primaryColors) {
        if (mRefreshHeader != null) {
     ...
     */
    @Override
    public VRefreshLayout setPrimaryColorsId(@ColorRes int... primaryColorId) {
        final View thisView = this;
     ...
     */
    @Override
    public VRefreshLayout setScrollBoundaryDecider(VScrollBoundaryDecider boundary) {
     ...
     */
    @Override
    public VRefreshLayout setNoMoreData(boolean noMoreData) {
        if (mState == VRefreshState.Refreshing && noMoreData) {
            finishRefreshWithNoMoreData();
        } else if (mState == VRefreshState.Loading && noMoreData) {
            finishLoadMoreWithNoMoreData();
        } else if (mFooterNoMoreData != noMoreData) {
            mFooterNoMoreData = noMoreData;
            if (mRefreshFooter instanceof VRefreshFooter) {
                if (((VRefreshFooter) mRefreshFooter).setNoMoreData(noMoreData)) {
                    ...
                    String msg = "Footer:" + mRefreshFooter + " NoMoreData is not supported.(不支持NoMoreData，请使用[VClassicsFooter]或者[自定义Footer并实现setNoMoreData方法且返回true])";
     ...
     */
    @Override
    public VRefreshLayout resetNoMoreData() {
     ...
     */
    @Override
    public VRefreshLayout finishRefresh() {
     ...
     */
    @Override
    public VRefreshLayout finishLoadMore() {
     ...
     */
    @Override
    public VRefreshLayout finishRefresh(int delayed) {
     ...
     */
    @Override
    public VRefreshLayout finishRefresh(boolean success) {
        if (success) {
     ...
     */
    @Override
    public VRefreshLayout finishRefresh(final int delayed, final boolean success, final Boolean noMoreData) {
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                if (count == 0) {
                    if (mState == VRefreshState.None && mViceState == VRefreshState.Refreshing) {
                        //autoRefresh 即将执行，但未开始
                        mViceState = VRefreshState.None;
                    } else if (reboundAnimator != null && mState.isHeader && (mState.isDragging || mState == VRefreshState.RefreshReleased)) {
                        //autoRefresh 正在执行，但未结束
                        //mViceState = VRefreshState.None;
                        /*
                         * 2020-3-15 BUG修复
                         * https://github.com/scwang90/VSmartRefreshLayout/issues/1019
                        ...
                        /*
                         * 2020-1-4 BUG修复
                         * https://github.com/scwang90/VSmartRefreshLayout/issues/1104
                         * 如果当前状态为 PullDownToRefresh 并且 mSpinner != 0
                         * mKernel.setState(VRefreshState.None); 内部会调用 animSpinner(0); 动画关闭
                        ...
                        if (mKernel.animSpinner(0) == null) {
                            ...
                            notifyStateChanged(VRefreshState.PullDownCanceled);
                        }
//                      mKernel.setState(VRefreshState.None);
                    } else if (mState == VRefreshState.Refreshing) {
                        count++;
                        mHandler.postDelayed(this, more);
                        //提前设置 状态为 RefreshFinish 防止 postDelayed 导致 finishRefresh 过后，外部判断 state 还是 Refreshing
                        notifyStateChanged(VRefreshState.RefreshFinish);
                    ...
                    int startDelay = 0;
                    if (mRefreshHeader != null) {
                        startDelay = mRefreshHeader.onFinish(VSmartRefreshLayout.this, success);
                        if (mOnMultiListener != null && mRefreshHeader instanceof VRefreshHeader) {
                            mOnMultiListener.onHeaderFinish((VRefreshHeader) mRefreshHeader, success);
                        }
                    }
                    ...
                    if (startDelay < Integer.MAX_VALUE) {
                        ...
                        if (mIsBeingDragged || mNestedInProgress) {
                            ...
                            if (mIsBeingDragged) {
                                ...
                                mTouchSpinner = 0;
                                mIsBeingDragged = false;
                                VSmartRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX, mLastTouchY + mSpinner - mTouchSlop * 2, 0));
                                VSmartRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mLastTouchY + mSpinner, 0));
                            }
                            if (mNestedInProgress) {
                                mTotalUnconsumed = 0;
                                VSmartRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_UP, mLastTouchX, mLastTouchY, 0));
                                mNestedInProgress = false;
                                mTouchSpinner = 0;
                            }
                        }
...
//                            resetStatus();
     ...
     */
    @Override
    public VRefreshLayout finishRefreshWithNoMoreData() {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
     ...
     */
    @Override
    public VRefreshLayout finishLoadMore(int delayed) {
     ...
     */
    @Override
    public VRefreshLayout finishLoadMore(boolean success) {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
     ...
     */
    @Override
    public VRefreshLayout finishLoadMore(final int delayed, final boolean success, final boolean noMoreData) {
        final int more = delayed >> 16;//动画剩余延时
        int delay = delayed << 16 >> 16;//用户指定延时
        Runnable runnable = new Runnable() {
            int count = 0;
            ...
            public void run() {
                if (count == 0) {
                    if (mState == VRefreshState.None && mViceState == VRefreshState.Loading) {
                        //autoLoadMore 即将执行，但未开始
                        mViceState = VRefreshState.None;
                    } else if (reboundAnimator != null && (mState.isDragging || mState == VRefreshState.LoadReleased) && mState.isFooter) {
                        ...
                        if (mKernel.animSpinner(0) == null) {
                            ...
                            notifyStateChanged(VRefreshState.PullUpCanceled);
                        }
                        //mKernel.setState(VRefreshState.None);
                    } else if (mState == VRefreshState.Loading && mRefreshFooter != null && mRefreshContent != null) {
                        count++;
                        mHandler.postDelayed(this, more);
                        //提前设置 状态为 LoadFinish 防止 postDelayed 导致 finishLoadMore 过后，外部判断 state 还是 Loading
                        notifyStateChanged(VRefreshState.LoadFinish);
                        return;
                    }
                    if (noMoreData) {
                        setNoMoreData(true);
                    ...
                    final int startDelay = mRefreshFooter.onFinish(VSmartRefreshLayout.this, success);
                    if (mOnMultiListener != null && mRefreshFooter instanceof VRefreshFooter) {
                        mOnMultiListener.onFooterFinish((VRefreshFooter) mRefreshFooter, success);
                    }
                    if (startDelay < Integer.MAX_VALUE) {
                        //计算布局将要移动的偏移量
                        ...
                        if (mIsBeingDragged || mNestedInProgress) {
                            ...
                            if (mIsBeingDragged) {
                                ...
                                VSmartRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX, mLastTouchY + offsetY + mTouchSlop * 2, 0));
                                VSmartRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mLastTouchY + offsetY, 0));
                            }
                            if (mNestedInProgress) {
                                mTotalUnconsumed = 0;
                                VSmartRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_UP, mLastTouchX, mLastTouchY, 0));
                        ...
                        mHandler.postDelayed(() -> {
                            ...
                            AnimatorListenerAdapter listenerAdapter = new AnimatorListenerAdapter() {
                                ...
                                public void onAnimationEnd(Animator animation) {
                                    ...
                                    if (mState == VRefreshState.LoadFinish) {
                                        notifyStateChanged(VRefreshState.None);
                                    }
                                }
                            };
                            if (mSpinner > 0) { //大于0表示下拉, 这是 Header 可见, Footer 不可见
                                ...
                                if (noMoreData && mEnableFooterFollowWhenNoMoreData) {//如果需要显示没有更多数据
                                    if (mSpinner >= -mFooterHeight) {//如果 Footer 的位置再可见范围内
                                        notifyStateChanged(VRefreshState.None);//直接通知重置状态,不关闭 Footer
     ...
     */
    @Override
    public VRefreshLayout finishLoadMoreWithNoMoreData() {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
     ...
     */
    @Override
    public VRefreshLayout closeHeaderOrFooter() {
        if (mState == VRefreshState.None && (mViceState == VRefreshState.Refreshing || mViceState == VRefreshState.Loading)) {
            //autoRefresh autoLoadMore 即将执行，但未开始
            mViceState = VRefreshState.None;
        }
        if (mState == VRefreshState.Refreshing) {
            finishRefresh();
        } else if (mState == VRefreshState.Loading) {
            finishLoadMore();
        } else {
    ...
    public boolean autoRefresh(int delayed, final int duration, final float dragRate,final boolean animationOnly) {
        if (mState == VRefreshState.None && isEnableRefreshOrLoadMore(mEnableRefresh)) {
            Runnable runnable = () -> {
                if (mViceState != VRefreshState.Refreshing) return;
                if (reboundAnimator != null) {
                    reboundAnimator.setDuration(0);//cancel会触发End调用，可以判断0来确定是否被cancel
                    reboundAnimator.cancel();//会触发 cancel 和 end 调用
                    reboundAnimator = null;
                ...
                reboundAnimator.setDuration(duration);
                reboundAnimator.setInterpolator(new VSmartUtil(VSmartUtil.INTERPOLATOR_VISCOUS_FLUID));
            ...
            setViceState(VRefreshState.Refreshing);
            if (delayed > 0) {
    ...
    public boolean autoLoadMore(int delayed, final int duration, final float dragRate, final boolean animationOnly) {
        if (mState == VRefreshState.None && (isEnableRefreshOrLoadMore(mEnableLoadMore) && !mFooterNoMoreData)) {
            Runnable runnable = () -> {
                if (mViceState != VRefreshState.Loading)return;
                if (reboundAnimator != null) {
                    reboundAnimator.setDuration(0);//cancel会触发End调用，可以判断0来确定是否被cancel
                    reboundAnimator.cancel();//会触发 cancel 和 end 调用
                    reboundAnimator = null;
            ...
            setViceState(VRefreshState.Loading);
            if (delayed > 0) {
                mHandler.postDelayed(runnable, delayed);
            } else {
                runnable.run();
     ...
     */
    public static void setDefaultRefreshHeaderCreator(@NonNull VDefaultRefreshHeaderCreator creator) {
     ...
     */
    public static void setDefaultRefreshFooterCreator(@NonNull VDefaultRefreshFooterCreator creator) {
     ...
     */
    public static void setDefaultRefreshInitializer(@NonNull VDefaultRefreshInitializer initializer) {
    ...
    public boolean isRefreshing() {
        return mState == VRefreshState.Refreshing;
    }
    ...
    public boolean isLoading() {
        return mState == VRefreshState.Loading;
    }
    //</editor-fold>
    //</editor-fold>

    //<editor-fold desc="核心接口 VRefreshKernel">

    /**
     * 刷新布局核心功能接口
     * 为功能复杂的 Header 或者 Footer 开放的接口
     */
    public class VRefreshKernelImpl implements VRefreshKernel {

        @NonNull
        @Override
        public VRefreshLayout getRefreshLayout() {
            return VSmartRefreshLayout.this;
        }

        @NonNull
        @Override
        public VRefreshContent getRefreshContent() {
            return mRefreshContent;
        }

        //<editor-fold desc="状态更改 state changes">
        @Override
        public VRefreshKernel setState(@NonNull VRefreshState state) {
            switch (state) {
                case None:
                    if (mState != VRefreshState.None && mSpinner == 0) {
                        notifyStateChanged(VRefreshState.None);
                    } else if (mSpinner != 0) {
                        animSpinner(0);
                    }
                ...
                case PullDownToRefresh:
                    if (!mState.isOpening && isEnableRefreshOrLoadMore(mEnableRefresh)) {
                        notifyStateChanged(VRefreshState.PullDownToRefresh);
                    } else {
                ...
                case PullUpToLoad:
                    if (isEnableRefreshOrLoadMore(mEnableLoadMore) && !mState.isOpening && !mState.isFinishing && !(mFooterNoMoreData && mEnableFooterFollowWhenNoMoreData && mFooterNoMoreDataEffective)) {
                        ...
                        setViceState(VRefreshState.PullUpToLoad);
                    }
                    break;
                case PullDownCanceled:
                    if (!mState.isOpening && isEnableRefreshOrLoadMore(mEnableRefresh)) {
                        notifyStateChanged(VRefreshState.PullDownCanceled);
                        setState(VRefreshState.None);
                    } else {
                        setViceState(VRefreshState.PullDownCanceled);
                    }
                    break;
                case PullUpCanceled:
                    if (isEnableRefreshOrLoadMore(mEnableLoadMore) && !mState.isOpening && !(mFooterNoMoreData && mEnableFooterFollowWhenNoMoreData && mFooterNoMoreDataEffective)) {
                        ...
                        setViceState(VRefreshState.PullUpCanceled);
                    }
                    break;
                case ReleaseToRefresh:
                    if (!mState.isOpening && isEnableRefreshOrLoadMore(mEnableRefresh)) {
                        notifyStateChanged(VRefreshState.ReleaseToRefresh);
                    } else {
                        setViceState(VRefreshState.ReleaseToRefresh);
                    }
                    break;
                case ReleaseToLoad:
                    if (isEnableRefreshOrLoadMore(mEnableLoadMore) && !mState.isOpening && !mState.isFinishing && !(mFooterNoMoreData && mEnableFooterFollowWhenNoMoreData && mFooterNoMoreDataEffective)) {
                        notifyStateChanged(VRefreshState.ReleaseToLoad);
                    } else {
                        setViceState(VRefreshState.ReleaseToLoad);
                    }
                    break;
                case ReleaseToTwoLevel: {
                    if (!mState.isOpening && isEnableRefreshOrLoadMore(mEnableRefresh)) {
                        notifyStateChanged(VRefreshState.ReleaseToTwoLevel);
                    } else {
                        setViceState(VRefreshState.ReleaseToTwoLevel);
                    }
                    break;
                }
                case RefreshReleased: {
                    if (!mState.isOpening && isEnableRefreshOrLoadMore(mEnableRefresh)) {
                        ...
                        setViceState(VRefreshState.RefreshReleased);
                    }
                    break;
                }
                case LoadReleased: {
                    if (!mState.isOpening && isEnableRefreshOrLoadMore(mEnableLoadMore)) {
                        ...
                        setViceState(VRefreshState.LoadReleased);
                    }
                    break;
                }
                case Refreshing:
        ...
        @Override
        public VRefreshKernel startTwoLevel(boolean open) {
            if (open) {
                AnimatorListenerAdapter listener = new AnimatorListenerAdapter() {
                    @Override
                    public void onAnimationEnd(Animator animation) {
                        ...
                        mKernel.setState(VRefreshState.TwoLevel);
                    }
                };
                final View thisView = VSmartRefreshLayout.this;
        ...
        @Override
        public VRefreshKernel finishTwoLevel() {
            if (mState == VRefreshState.TwoLevel) {
                mKernel.setState(VRefreshState.TwoLevelFinish);
                if (mSpinner == 0) {
                    moveSpinner(0, false);
                    notifyStateChanged(VRefreshState.None);
                } else {
         ...
         */
        public VRefreshKernel moveSpinner(final int spinner, final boolean isDragging) {
            if (mSpinner == spinner
            ...
            if (mRefreshContent != null) {
                ...
                if (changed) {
                    ...
                    if (mFooterNoMoreData && mFooterNoMoreDataEffective && mEnableFooterFollowWhenNoMoreData
                            && mRefreshFooter instanceof VRefreshFooter && mRefreshFooter.getSpinnerStyle() == VSpinnerStyle.Translate
                            && isEnableRefreshOrLoadMore(mEnableLoadMore)) {
                        mRefreshFooter.getView().setTranslationY(Math.max(0, tSpinner));
                    }
                    boolean header = mEnableClipHeaderWhenFixedBehind && mRefreshHeader != null && mRefreshHeader.getSpinnerStyle() == VSpinnerStyle.FixedBehind;
                    header = header || mHeaderBackgroundColor != 0;
                    boolean footer = mEnableClipFooterWhenFixedBehind && mRefreshFooter != null && mRefreshFooter.getSpinnerStyle() == VSpinnerStyle.FixedBehind;
            ...
            if ((spinner >= 0 || oldSpinner > 0) && mRefreshHeader != null) {
                ...
                if (isEnableRefreshOrLoadMore(mEnableRefresh) || (mState == VRefreshState.RefreshFinish && !isDragging)) {
...
//                            mRefreshHeader.getView().requestLayout();
                ...
                if (oldSpinner != mSpinner && mOnMultiListener != null && mRefreshHeader instanceof VRefreshHeader) {
                    mOnMultiListener.onHeaderMoving((VRefreshHeader) mRefreshHeader, isDragging, percent, offset, headerHeight, maxDragHeight);
                }
            ...
            if ((spinner <= 0 || oldSpinner < 0) && mRefreshFooter != null) {
                ...
                if (isEnableRefreshOrLoadMore(mEnableLoadMore) || (mState == VRefreshState.LoadFinish && !isDragging)) {
                    if (oldSpinner != mSpinner) {
                        if (mRefreshFooter.getSpinnerStyle() == VSpinnerStyle.Translate) {
                            mRefreshFooter.getView().setTranslationY(mSpinner);
...
//                            mRefreshFooter.getView().requestLayout();
                ...
                if (oldSpinner != mSpinner && mOnMultiListener != null && mRefreshFooter instanceof VRefreshFooter) {
                    mOnMultiListener.onFooterMoving((VRefreshFooter)mRefreshFooter, isDragging, percent, offset, footerHeight, maxDragHeight);
                }
            }
            return this;
        }
        ...
        public ValueAnimator animSpinner(int endSpinner) {
            return VSmartRefreshLayout.this.animSpinner(endSpinner, 0, mReboundInterpolator, mReboundDuration);
        }
        //</editor-fold>
        ...
        @Override
        public VRefreshKernel requestDrawBackgroundFor(@NonNull VRefreshComponent internal, int backgroundColor) {
        ...
        @Override
        public VRefreshKernel requestNeedTouchEventFor(@NonNull VRefreshComponent internal, boolean request) {
            if (internal.equals(mRefreshHeader)) {
        ...
        @Override
        public VRefreshKernel requestDefaultTranslationContentFor(@NonNull VRefreshComponent internal, boolean translation) {
            if (internal.equals(mRefreshHeader)) {
        ...
        @Override
        public VRefreshKernel requestRemeasureHeightFor(@NonNull VRefreshComponent internal) {
            if (internal.equals(mRefreshHeader)) {
                if (mHeaderHeightStatus.notified) {
                    mHeaderHeightStatus = mHeaderHeightStatus.unNotify();
                }
        ...
        @Override
        public VRefreshKernel requestFloorDuration(int duration) {
            mFloorDuration = duration;
            return this;
        }

        @Override
        public VRefreshKernel requestFloorBottomPullUpToCloseRate(float rate) {
            mTwoLevelBottomPullUpToCloseRate = rate;
            return this;
        }

        @Override
        public VRefreshKernel onAutoRefreshAnimationEnd(Animator animation, boolean animationOnly) {
            if (animation != null && animation.getDuration() == 0) {
                return this;//0 表示被取消
            }
            reboundAnimator = null;
            if (mState != VRefreshState.ReleaseToRefresh) {
                this.setState(VRefreshState.ReleaseToRefresh);
            }
            VSmartRefreshLayout.this.setStateRefreshing(!animationOnly);
            return this;
        }

        @Override
        public VRefreshKernel onAutoLoadMoreAnimationEnd(Animator animation, boolean animationOnly) {
            if (animation != null && animation.getDuration() == 0) {
                return this;//0 表示被取消
            }
            reboundAnimator = null;
            if (mRefreshFooter != null) {
                if (mState != VRefreshState.ReleaseToLoad) {
                    this.setState(VRefreshState.ReleaseToLoad);
                }
                VSmartRefreshLayout.this.setStateLoading(!animationOnly);
            } else {
                /*
                 * 2019-12-24 修复 mRefreshFooter=null 时状态错乱问题
                 */
                this.setState(VRefreshState.None);
            }
            return this;
        }
