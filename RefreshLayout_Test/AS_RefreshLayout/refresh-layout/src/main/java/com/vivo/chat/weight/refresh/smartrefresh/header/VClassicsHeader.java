...
import com.vivo.chat.weight.refresh.smartrefresh.drawable.VProgressDrawable;
import com.vivo.chat.weight.refresh.smartrefresh.classics.VArrowDrawable;
import com.vivo.chat.weight.refresh.smartrefresh.classics.VClassicsAbstract;
...
public class VClassicsHeader extends VClassicsAbstract<VClassicsHeader> implements VRefreshHeader {
...
//    public static String REFRESH_HEADER_UPDATE = "'Last update' M-d HH:mm";
    ...
    public VClassicsHeader(Context context, AttributeSet attrs) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.VClassicsHeader);
        ...
        lpUpdateText.topMargin = ta.getDimensionPixelSize(R.styleable.VClassicsHeader_srlTextTimeMarginTop, VSmartUtil.dp2px(0));
        lpProgress.rightMargin = ta.getDimensionPixelSize(R.styleable.VClassicsHeader_srlDrawableMarginRight, VSmartUtil.dp2px(20));
        lpArrow.rightMargin = lpProgress.rightMargin;

        lpArrow.width = ta.getLayoutDimension(R.styleable.VClassicsHeader_srlDrawableArrowSize, lpArrow.width);
        lpArrow.height = ta.getLayoutDimension(R.styleable.VClassicsHeader_srlDrawableArrowSize, lpArrow.height);
        lpProgress.width = ta.getLayoutDimension(R.styleable.VClassicsHeader_srlDrawableProgressSize, lpProgress.width);
        lpProgress.height = ta.getLayoutDimension(R.styleable.VClassicsHeader_srlDrawableProgressSize, lpProgress.height);

        lpArrow.width = ta.getLayoutDimension(R.styleable.VClassicsHeader_srlDrawableSize, lpArrow.width);
        lpArrow.height = ta.getLayoutDimension(R.styleable.VClassicsHeader_srlDrawableSize, lpArrow.height);
        lpProgress.width = ta.getLayoutDimension(R.styleable.VClassicsHeader_srlDrawableSize, lpProgress.width);
        lpProgress.height = ta.getLayoutDimension(R.styleable.VClassicsHeader_srlDrawableSize, lpProgress.height);

        mFinishDuration = ta.getInt(R.styleable.VClassicsHeader_srlFinishDuration, mFinishDuration);
        mEnableLastTime = ta.getBoolean(R.styleable.VClassicsHeader_srlEnableLastTime, mEnableLastTime);
        mSpinnerStyle = VSpinnerStyle.values[ta.getInt(R.styleable.VClassicsHeader_srlClassicsSpinnerStyle,mSpinnerStyle.ordinal)];

        if (ta.hasValue(R.styleable.VClassicsHeader_srlDrawableArrow)) {
            mArrowView.setImageDrawable(ta.getDrawable(R.styleable.VClassicsHeader_srlDrawableArrow));
        } else if (mArrowView.getDrawable() == null){
            mArrowDrawable = new VArrowDrawable();
            mArrowDrawable.setColor(0xff666666);
            mArrowView.setImageDrawable(mArrowDrawable);
        }

        if (ta.hasValue(R.styleable.VClassicsHeader_srlDrawableProgress)) {
            mProgressView.setImageDrawable(ta.getDrawable(R.styleable.VClassicsHeader_srlDrawableProgress));
        } else if (mProgressView.getDrawable() == null) {
            mProgressDrawable = new VProgressDrawable();
            mProgressDrawable.setColor(0xff666666);
            mProgressView.setImageDrawable(mProgressDrawable);
        }

        if (ta.hasValue(R.styleable.VClassicsHeader_srlTextSizeTitle)) {
            mTitleText.setTextSize(TypedValue.COMPLEX_UNIT_PX, ta.getDimensionPixelSize(R.styleable.VClassicsHeader_srlTextSizeTitle, VSmartUtil.dp2px(16)));
//        } else {
//            mTitleText.setTextSize(16);
        }

        if (ta.hasValue(R.styleable.VClassicsHeader_srlTextSizeTime)) {
            mLastUpdateText.setTextSize(TypedValue.COMPLEX_UNIT_PX, ta.getDimensionPixelSize(R.styleable.VClassicsHeader_srlTextSizeTime, VSmartUtil.dp2px(12)));
//        } else {
//            mLastUpdateText.setTextSize(12);
        }

        if (ta.hasValue(R.styleable.VClassicsHeader_srlPrimaryColor)) {
            super.setPrimaryColor(ta.getColor(R.styleable.VClassicsHeader_srlPrimaryColor, 0));
        }
        if (ta.hasValue(R.styleable.VClassicsHeader_srlAccentColor)) {
            setAccentColor(ta.getColor(R.styleable.VClassicsHeader_srlAccentColor, 0));
        }

        if(ta.hasValue(R.styleable.VClassicsHeader_srlTextPulling)){
            mTextPulling = ta.getString(R.styleable.VClassicsHeader_srlTextPulling);
        ...
        if(ta.hasValue(R.styleable.VClassicsHeader_srlTextLoading)){
            mTextLoading = ta.getString(R.styleable.VClassicsHeader_srlTextLoading);
        ...
        if(ta.hasValue(R.styleable.VClassicsHeader_srlTextRelease)){
            mTextRelease = ta.getString(R.styleable.VClassicsHeader_srlTextRelease);
        ...
        if(ta.hasValue(R.styleable.VClassicsHeader_srlTextFinish)){
            mTextFinish = ta.getString(R.styleable.VClassicsHeader_srlTextFinish);
        ...
        if(ta.hasValue(R.styleable.VClassicsHeader_srlTextFailed)){
            mTextFailed = ta.getString(R.styleable.VClassicsHeader_srlTextFailed);
        ...
        if(ta.hasValue(R.styleable.VClassicsHeader_srlTextSecondary)){
            mTextSecondary = ta.getString(R.styleable.VClassicsHeader_srlTextSecondary);
        ...
        if(ta.hasValue(R.styleable.VClassicsHeader_srlTextRefreshing)){
            mTextRefreshing = ta.getString(R.styleable.VClassicsHeader_srlTextRefreshing);
        ...
        if(ta.hasValue(R.styleable.VClassicsHeader_srlTextUpdate)){
            mTextUpdate = ta.getString(R.styleable.VClassicsHeader_srlTextUpdate);
        ...
        mShared = context.getSharedPreferences("VClassicsHeader", Context.MODE_PRIVATE);
        setLastUpdateTime(new Date(mShared.getLong(KEY_LAST_UPDATE_TIME, System.currentTimeMillis())));

    }
    //</editor-fold>

    //<editor-fold desc="VRefreshHeader">
    @Override
    public int onFinish(@NonNull VRefreshLayout layout, boolean success) {
        if (success) {
    ...
    public VClassicsHeader setLastUpdateTime(Date time) {
        final View thisView = this;
        mLastTime = time;
    ...
    public VClassicsHeader setTimeFormat(DateFormat format) {
    ...
    public VClassicsHeader setLastUpdateText(CharSequence text) {
        mLastTime = null;
        mLastUpdateText.setText(text);
        return this;
    }

    public VClassicsHeader setAccentColor(@ColorInt int accentColor) {
        mLastUpdateText.setTextColor(accentColor&0x00ffffff|0xcc000000);
        return super.setAccentColor(accentColor);
    }

    public VClassicsHeader setEnableLastTime(boolean enable) {
        final View updateView = mLastUpdateText;
    ...
    public VClassicsHeader setTextSizeTime(float size) {
    ...
    public VClassicsHeader setTextSizeTime(int unit, float size) {
    ...
    public VClassicsHeader setTextTimeMarginTop(float dp) {
        final View updateView = mLastUpdateText;
        ViewGroup.MarginLayoutParams lp = (ViewGroup.MarginLayoutParams)updateView.getLayoutParams();
        lp.topMargin = VSmartUtil.dp2px(dp);
        updateView.setLayoutParams(lp);
        return this;
    }

    public VClassicsHeader setTextTimeMarginTopPx(int px) {
